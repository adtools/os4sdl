...
os4video_InitOSKeymap(_THIS)
{
	...
	map(0x64, SDLK_LALT);
	map(0x66, SDLK_LSUPER);
	map(0x40, SDLK_SPACE);
	map(0x67, SDLK_RSUPER);
	map(0x65, SDLK_RALT);
	map(0x0f, SDLK_KP0);
...
os4video_KMod(uint16 Qualifier)
{
	...
	if (Qualifier & IEQUALIFIER_LCOMMAND)	x |= KMOD_LMETA;
	if (Qualifier & IEQUALIFIER_RCOMMAND)	x |= KMOD_RMETA;
...
static void
os4video_HandleMouseMotion(struct MyIntuiMessage *imsg)
{
	uint8 buttonstate = 0;
	...
	if (imsg->wantDelta)
	{
		if (imsg->MouseDX != 0 || imsg->MouseDY != 0)
		{
			...
			SDL_PrivateMouseMotion(buttonstate, 1, imsg->MouseDX, imsg->MouseDY);
		}
	}
	else
	{
		...
		SDL_PrivateMouseMotion(buttonstate, 0, imsg->PointerX, imsg->PointerY);
	}
}
...
os4video_HandleEnterLeaveWindow (SDL_VideoDevice *_this, struct MyIntuiMessage *imsg)
{
	...
	if (new_state != hidden->pointerState)
	{
		...
		if (new_state == pointer_inside_window)
		{
			...
			os4video_HandleMouseMotion(imsg);
	...
	while (!bailOut && (NULL != (imsg = (struct IntuiMessage *)IExec->GetMsg(hidden->userPort))))
	{
		...
		case IDCMP_MOUSEMOVE:
			if (hidden->pointerState == pointer_inside_window)
				os4video_HandleMouseMotion(&msg);
		...
		case IDCMP_INTUITICKS:
			if (!hidden->scr && hidden->windowActive && (_this->input_grab == SDL_GRAB_ON))
			{
...
SDL_GrabMode
os4video_GrabInput(SDL_VideoDevice *_this, SDL_GrabMode mode)
{
	struct SDL_PrivateVideoData *hidden = _this->hidden;
	...
	if (hidden->win) {
		...
		if (!hidden->scr && hidden->windowActive && (mode == SDL_GRAB_ON))
			os4video_activatePointerGrab(hidden->win, SDL_GRAB_ON);
		else if (mode == SDL_GRAB_OFF)
			os4video_activatePointerGrab(hidden->win, SDL_GRAB_OFF);
...
void os4video_CheckMouseMode(SDL_VideoDevice *_this)
{
	 ...
	 */
	hidden->isMouseRelative = (!(SDL_cursorstate == CURSOR_VISIBLE) && (_this->input_grab != SDL_GRAB_OFF));
